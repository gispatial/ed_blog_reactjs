...
var Parse5RichTextParser = /** @class */ (function () {
    ...
    Parse5RichTextParser.prototype.resolveRichTextElement = function (contentItemCodename, html, elementName, replacement, config) {
        // create document
        var documentFragment = parseFragment(html);
        ...
        var resolvedHtml = serialize(documentFragment);
        return {
            links: result.links,
    ...
    Parse5RichTextParser.prototype.processImage = function (contentItemCodename, elementName, element, replacement, config, result) {
        ...
        var dataImageIdAttribute = attributes.find(function (m) { return m.name === parserConfiguration.imageElementData.dataImageId; });
        ...
        // set url of image
        var srcAttribute = attributes.find(function (m) { return m.name === parserConfiguration.imageElementData.srcAttribute; });
        if (srcAttribute) {
    ...
    Parse5RichTextParser.prototype.processLink = function (element, replacement, config, result) {
        ...
        var dataItemIdAttribute = attributes.find(function (m) { return m.name === parserConfiguration.linkElementData.dataItemId; });
        ...
        if (urlSlugResult.html) {
            ...
            var linkHtml = urlSlugResult.html
                ...
                : '';
            if (linkHtml) {
                var linkRootNodes = parseFragment(linkHtml).childNodes;
    ...
    Parse5RichTextParser.prototype.processModularContentItem = function (elementName, element, replacement, config, result) {
        var _this = this;
        var attributes = element.attrs;
        var dataTypeAttribute = attributes.find(function (m) { return m.name === parserConfiguration.modularContentElementData.dataType; });
        ...
        if (dataTypeAttribute && !resolvedDataAttribute) {
            ...
            if (dataTypeAttribute.value === 'item') {
                ...
                var dataCodenameAttribute = attributes.find(function (m) { return m.name === parserConfiguration.modularContentElementData.dataCodename; });
                ...
                var relAttribute = attributes.find(function (m) { return m.name === parserConfiguration.modularContentElementData.relAttribute; });
                ...
                var serializedChildNodes = parseFragment(resultHtml);
                // add child nodes
